...
class TrainDataSetTest(Dataset):
    ...
    def _compute_length(self):
        ...
        while index + inputs_targets_len < len(self.image_files):
            while self.image_files[index].split(self.split)[0] != \
                    self.image_files[index + inputs_targets_len].split(self.split)[0]:
                index += 1
    ...
    def __getitem__(self, index):
        ...
        targets = []
        while self.image_files[index].split(self.split)[0] != \
                self.image_files[index + arg_config.inputs_len + arg_config.targets_len - 1].split(self.split)[0]:
            index += 1
        self.index = index + 1
        for _ in range(arg_config.inputs_len):
            img = cv2.resize(cv2.imread(os.path.join(self.images_path, self.image_files[index])), (img_size, img_size))
