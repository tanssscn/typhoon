...
def train_one_epoch(model, optimizer, data_loader, device, epoch):
    ...
    for step, data in enumerate(data_loader):
        ...
        img_len = outputs.shape[1]
        img_batch = outputs.shape[0]
        for i in range(img_batch):
            for j in range(img_len):
                ii = outputs[i, j, :].clone().cpu()
                ii = ii.permute(1, 2, 0) * 255.
                util.save_image(ii.detach().numpy(), f'{arg_config.root}runs/testing/pred/{i}{j}.png')
                ii = targets[i, j, :]
                ii = ii.permute(1, 2, 0) * 255.
                util.save_image(ii.numpy(), f'{arg_config.root}runs/testing/pred/{i}{j}-target.png')
        targets = targets.to(device)
        loss = l1_loss(outputs, targets) + l2_loss(outputs, targets)
        loss.backward()
        optimizer.step()
